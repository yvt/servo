            ...
            self.window
                ...
                .queue(
                    task!(completely_loaded: move || {
                        ...
                        //
                        // For `InitialAboutBlank`, these "parsing" steps are a
                        // purely internal artifact that isn't supposed to
                        // happen as per spec, so don't do
                        // `notify_constellation_load`. The document is already
                        // "completely loaded", and the "completely finish
                        // loading" steps have already run in this case.
                        if document.source != DocumentSource::InitialAboutBlank {
...
#[derive(MallocSizeOf, PartialEq, JSTraceable)]
