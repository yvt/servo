...
/// A sequence number generated by a script thread. The constellation attaches
/// the last seen `FocusSequenceNumber` to every focus-related message it sends.
///
...
/// iframe is still focused. The root cause is that the `FocusDocument` message
/// sent by the constellation is out of date by the time it's processed by the
/// script thread.
///
/// This can be resolved by associating a sequence number to each message.
/// Whenever a script thread initiates a focus operation, it generates and sends
/// a brand new sequence number. The constellation attaches the last-received
/// sequence number to each message it sends. This way, the script thread can
/// discard out-dated incoming focus messages, and eventually, all actors
/// converge to the consistent state based on the last focus message received by
/// the constellation.
///
/// ```text
/// script                            constellation
